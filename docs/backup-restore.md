...
Argus keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/argus-backup` tool
...
```bash
# Create a snapshot → backups/argus-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/argus-backup snapshot

# List existing snapshots (most recent first)
./scripts/argus-backup list

# Check a tarball's integrity without extracting it
./scripts/argus-backup verify backups/argus-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/argus-backup restore backups/argus-backup-20260101-120000.tar.gz --yes
```
...
./scripts/argus-backup snapshot --out /mnt/nas/argus-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/argus-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/argus && ./scripts/argus-backup snapshot --out "/mnt/nas/argus-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `argus-backup` therefore does not capture the Docker
