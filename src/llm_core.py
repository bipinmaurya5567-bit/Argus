...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Argus' canonical OpenAI-style messages to native Ollama /api/chat.

    Argus carries assistant tool calls in the OpenAI shape, where
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Argus session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/argus")
        h.setdefault("X-OpenRouter-Title", "Argus")
    if provider == "copilot":
...
# value returns HTTP 400. Argus does not currently send the `thinking` mode
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Argus-only metadata before sending messages to providers.
