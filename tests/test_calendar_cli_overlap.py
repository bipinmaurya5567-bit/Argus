"""Regression: `argus-calendar list` must select events that OVERLAP the
...
`argus-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "argus-calendar"
    loader = importlib.machinery.SourceFileLoader("argus_calendar_cli", str(path))
