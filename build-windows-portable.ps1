...
<#
  Build a portable Windows distribution for Argus.

  Output layout:
    dist\Argus\Argus.exe
    dist\Argus\static\...
    dist\Argus\scripts\...
    dist\Argus\mcp_servers\...
    dist\Argus\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Argus @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Argus" -ForegroundColor Green
