                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Argus (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Argus" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "argus_kind" if "argus_kind" in cols else "'scheduled' AS argus_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                argus_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Argus-Origin"] = "argus-ui"
                outer["X-Argus-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", argus_kind or "scheduled")[:64]
                outer["X-Argus-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `argus-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ARGUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `argus-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ARGUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ARGUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ARGUS_INPROCESS_POLLERS=0); "
            "drive `argus-mail poll-scheduled` externally."
        )
        return
    import asyncio
